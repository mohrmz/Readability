...
def name_recommendation(test_dataX,test_dataY,neigh, learned_dataY,type):
    ...
    for recommended in recommended_names:
        ...
        if RecommendedMethodName.lower() in ['get','set'] and i<10:
            continue
        RecommendedMethodNamechars = token_to_words(RecommendedMethodName)
        RecommendedNamePath = learned_dataY.iloc[[recommended], 5].values[0]
        ...
        if UseKNeighborsClassifier:
            ...
            ListResult.append([ExpectedMethodID,ExpectedMethodName,RecommendedMethodName,RecommendedMethodID,WuPalmerScore,Precision,Recall,f1score,RecommendedNamePath])
