# key: name smell file, value: name smell method , type = object type 

name_smells={
     'AbbreviatedNames' :['abbreviated_names','']
    ,'AbstractNames':['abstract_names','']
    ,'BadClassNamesConvention':['bad_class_names_convention','Class']
    ,'BadMethodNamesConvention':['bad_method_names_convention','Method']
    ,'DisinformativeName':['disinformative_name','']
    ,'EncodedName':['encoded_name','']
    ,'InconsistentParts' :['inconsistent_parts','']
    ,'InconsistentWithMethodParameters':['inconsistent_with_method_parameters','Other']
    ,'InconsistentWithOtherClassMethods':['inconsistent_with_other_class_methods','Other']
    ,'MeaninglessNames':['meaningless_names','']
    ,'MentalMappingName':['mental_mapping_name','Other']
    ,'MultipleWords':['multiple_words','']
    ,'NonpronounceableName': ['nonpronounceable_name','']
    ,'NonsearchableName':['nonsearchable_name','']
    ,'NumericSuffix':['numeric_suffix','']
    ,'ShortNames':['short_names',''] 
    ,'VagueWords':['vague_words','']
    ,'VestigialHungarianNotation':['vestigial_hungarian_notation',''] 
    ,'WrongNames':['wrong_names','']    
}